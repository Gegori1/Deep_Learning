...
def intersection_over_union(b_predict: torch.Tensor, b_truth: torch.Tensor, format: str="cxcywh", reduction: str="sum", check_true: bool=False):
    ...
    # assert torch.all(b_predict[..., 2] > b_predict[..., 0]), "Bounding box width must be positive"
    # assert torch.all(b_predict[..., 3] > b_predict[..., 1]), "Bounding box height must be positive"
...
if __name__ == '__main__':
    ...
    iou = intersection_over_union(
        y_pred, y_real, 
        format="cxcywh", reduction="mean", check_true=True
    )
